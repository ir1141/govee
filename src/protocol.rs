...
pub fn send_command(ip: &str, cmd: &str, data: serde_json::Value, debug: bool) -> Option<serde_json::Value> {
    ...
    let sock = socket2::Socket::new(socket2::Domain::IPV4, socket2::Type::DGRAM, Some(socket2::Protocol::UDP)).ok()?;
    sock.set_reuse_address(true).ok()?;
    sock.bind(&socket2::SockAddr::from(std::net::SocketAddrV4::new(std::net::Ipv4Addr::UNSPECIFIED, RESPONSE_PORT))).ok()?;
    sock.set_read_timeout(Some(std::time::Duration::from_secs(2))).ok()?;
    let sock: UdpSocket = sock.into();
    sock.send_to(&msg, (ip, CONTROL_PORT)).ok()?;

    if cmd == "devStatus" {
        let mut buf = [0u8; 4096];
        match sock.recv_from(&mut buf) {
            Ok((n, _)) => {
                let resp_str = std::str::from_utf8(&buf[..n]).ok()?;
                if debug {
                    eprintln!("  << {}", resp_str);
                }
                let resp: GoveeMsg = serde_json::from_str(resp_str).ok()?;
                Some(resp.msg.data)
            }
            Err(_) => None,
        }
...
pub fn send_segments(ip: &str, colors: &[(u8, u8, u8)], gradient: bool) -> std::io::Result<()> {
    assert!(colors.len() <= 255, "too many segments (max 255)");
