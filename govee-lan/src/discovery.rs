...
pub fn scan_devices(timeout: Duration) -> Result<Vec<DeviceInfo>> {
    ...
    let recv_sock = match (|| -> std::io::Result<UdpSocket> {
        ...
        Ok(s) => s,
        Err(e) => {
            anyhow::bail!("Failed to bind discovery port {RESPONSE_PORT}: {e}");
        }
    };
    ...
    if let Ok(send_sock) = UdpSocket::bind("0.0.0.0:0") {
        ...
        send_sock
            .send_to(&scan_msg, (MULTICAST_GROUP, SCAN_PORT))
            .ok();
    }
                    ...
                    if let Ok(resp) = serde_json::from_str::<GoveeMsg>(text) {
                        if let Ok(info) = serde_json::from_value::<DeviceInfo>(resp.msg.data) {
                            if !info.ip.is_empty()
                                ...
                                && !devices.iter().any(|d: &DeviceInfo| d.ip == info.ip)
                            {
                                devices.push(info);
                            }
                        }
...
pub fn discover_device(timeout: Duration) -> Option<String> {
    scan_devices(timeout).ok()?.into_iter().next().map(|d| d.ip)
}
